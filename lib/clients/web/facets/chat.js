...
/**
 * Execute a slash command on channel
 * @see {@link https://github.com/ErikKalkoken/slackApiDoc/blob/master/chat.command.md}
 *
 * @param {?} channel - Channel where to execute the command
 * @param {Object=} opts
 * @param {?} opts.text - Pass arguments to the command
 */
ChatFacet.prototype.command = function command(channel, command, opts) {
  var requiredArgs = {
    channel: channel,
    command: command
  };

  return this.makeAPICall('chat.command', requiredArgs, opts);
 };
